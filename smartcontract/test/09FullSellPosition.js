...
contract("Test TuneTrader Exchange Contract Basic Tests ", async (accounts) => {
  ...
  it('03. Add Song to Exchange.', async () => {
    ...
    var volume = 200 
    await expect(TuneTraderExchange.AddPosition(songToken.address, volume, buySell, cost, {from: accounts[2], gasPrice: 1} )).to.be.eventually.fulfilled
  ...
  it('05. Transfer tokens to Position to actitvate it.', async () => {
      var volume = 200
      await expect (songToken.transfer(TTManagerContract.address,  volume,{from:accounts[2]})).to.be.eventually.fulfilled
  ...
  it('06. Send Ether to receive tokens', async () => {
    ...
    var sellerBalanceBefore = web3.eth.getBalance(accounts[2])

    let tx = await expect(TTManagerContract.BuyTokens({value:cost, gasPrice:1, from: accounts[1]})).to.be.eventually.fulfilled
    ...
    expect(managerTokenBalanceBefore.toNumber()).to.be.equal(200)
    expect(buyerTokenBalanceBefore.toNumber()).to.be.equal(0)
    expect(sellerTokenBalanceBefore.toNumber()).to.be.equal(2000 - volume)
  
    expect(managerTokenBalanceAfter.toNumber()).to.be.equal(0)
    ...
    var sellerBalance = web3.eth.getBalance(accounts[2])

    // expect (balance).to.be.equal(balanceBefore.sub(cost.toString()).sub(tx.gasUsed))
    expect (balance.toString()).to.be.equal(balanceBefore.sub(cost).sub(tx.receipt.gasUsed).toString())
    expect (sellerBalance.toNumber()).to.be.equal(sellerBalanceBefore.plus(cost).toNumber())
    console.log('Balance: ', web3.fromWei(balance, "ether").toFormat(2))
