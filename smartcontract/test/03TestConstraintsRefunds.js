...
contract("Test TuneTrader Contract for maxCap constrain and minpresale constrain and refund. ",   async (accounts)=> {
  ...
  // function AddICO(address _wallet,uint256 _teamTokens,uint256[] constraints, uint256 _price, uint256 _durationDays, uint _presaleduration,uint8[] _bonuses,uint256 assignedTokens) public
    ...
    it("03. Adding ICO with added song for ICO should be fulfiled", async () => {
      await TuneTraderContract.AddICO(saleWallet,teamTokens,constraints,rate,10,10,bonuses,assignTokens,{"gasPrice": 1})
      ...
      saleInstance = SongCrowdsale.at(myICO)
    ...
    it("04. Should be possible to buy tokens first second of presale.10% bonus applies.", async () => {
      ...
      await saleInstance.sendTransaction ({"gas":914366353, "gasPrice":1, "value":8,"from":accounts[5]})
    ...
    it("05. Should not be possible to buy tokens exceeding maxCap", function ()  {
      ...
      return expect (saleInstance.sendTransaction ({"gas":914366353, "gasPrice":1, "value":2,"from":accounts[5]})).to.be.eventually.rejected
    })
    ...
    it("07.It should be not possible to buy tokens in refund state.First transaction should be fulfilled. Account balance should be only subtracted by transaction cost.", function ()  {
      let initBalance = web3.toBigNumber(web3.eth.getBalance(accounts[5]))
      // console.log('Initial Balance:', initBalance)
      return saleInstance.sendTransaction ({"gas":914366353, "gasPrice":1, "value":5000,"from":accounts[5]}).then(function(res) {
        // console.log(res)
        let curBalance = (web3.eth.getBalance(accounts[5]))
        // console.log('Cur Balance: ', curBalance, " Gas Used: ",res.receipt.gasUsed)
        expect (curBalance.toString()).to.be.equal(initBalance.minus(res.receipt.gasUsed).toString())
        // expect (res.receipt.status).to.be.equal('0x01')
      })
    })

    it("08.It should be not possible to buy tokens in refund state.Second transaction should be rejected.", function ()  {
    ...
    it("09. Since two buying transaction failed. Token balance shouldn't change. ", async () => {
      ...
      expect(parseInt(curBalance)).to.be.equal(myBalance)
    })
    ...
    it("10. It should be possible to ask for refund.This should return 8 wei", async () => {
      ...
      console.log(tx)
      return expect(curBalance.toString()).to.be.equal(initBalance.plus(8).minus(tx.receipt.gasUsed).toString())
    })
